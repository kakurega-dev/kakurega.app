...
>
	...
	<div v-else ref="rootEl" class="_gaps">
		<div v-show="pagination.reversed && more" key="_more_">
			<MkButton v-if="!moreFetching" v-appear="(enableInfiniteScroll && !props.disableAutoLoad) ? () => fetchMore(true, true) : null" :class="$style.more" :wait="moreFetching" primary rounded @click="() => fetchMore(true, false)">
				{{ i18n.ts.loadMore }}
			</MkButton>
			<MkLoading v-else/>
		</div>
		...
		<div v-show="!pagination.reversed && more" key="_more_">
			<MkButton v-if="!moreFetching" v-appear="(enableInfiniteScroll && !props.disableAutoLoad) ? () => fetchMore(false, true) : null" :class="$style.more" :wait="moreFetching" primary rounded @click="() => fetchMore(false, false)">
				{{ i18n.ts.loadMore }}
			</MkButton>
			<MkLoading v-else/>
		</div>
