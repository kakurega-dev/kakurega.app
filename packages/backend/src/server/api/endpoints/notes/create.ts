		...
		super(meta, paramDef, async (ps, me) => {
			try {
				const note = await this.noteCreateService.fetchAndCreate(me, {
					...
					deleteAt: ps.scheduledDelete ? ps.scheduledDelete.deleteAfter ? new Date(Date.now() + ps.scheduledDelete.deleteAfter) : ps.scheduledDelete.deleteAt ? new Date(ps.scheduledDelete.deleteAt) : null : null,
				});
				...
				if (err instanceof IdentifiableError) {
					if (err.id === '689ee33f-f97c-479a-ac49-1b9f8140af99') {
						...
						throw new ApiError(meta.errors.noSuchChannel);
					} else if (err.id === '4ef48daf-141c-40f1-83c5-20d61695d427') {
						throw new ApiError(meta.errors.cannotScheduleDeleteEarlierThanNow);
					} else if (err.id === 'a1376a20-16e7-4151-af13-0b005b471d71') {
